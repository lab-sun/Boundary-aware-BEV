...
from diffBEV.src.LatentDiffusion_deeplab_GPU3 import LatentDiffusionConditional
...
if __name__ == '__main__':
    ...
    log_root = './logs/DiffBEV_GPU3'
    log_frequency = 10
    ckpt_root = './pretrain_weights/DiffBEV_GPU3/'
    save_frequency = 1
    ...
    logger = TensorBoardLogger('logs', name='DiffBEV_GPU3/{}/'.format(create_time))
    ...
    checkpoint_callback = ModelCheckpoint(save_top_k=-1,
                                          mode='max', # 趋势越大越好
                                          dirpath="./logs/DiffBEV_GPU3/{}/checkpoints/".format(create_time), 
                                          monitor="val_iou", 
    ...
    trainer.fit(model)
    # trainer.fit(model, ckpt_path="./logs/DiffBEV_GPU3/2023-11-24-03-06/checkpoints/last.ckpt")

    trainer.test(model, dataloaders=test_loader)

    # save model
    ckpt_path = './pretrain_weights/DiffBEV_GPU3/{}/'.format(create_time)
    if not os.path.exists(ckpt_path):
        os.makedirs(ckpt_path)
    model_save_path = os.path.join(ckpt_path, 'DiffBEV_GPU3.pth')# './pretrain_weights/ae_epoch{}.pth'.format(epoch) #os.path.join('./pretrain_weights/', 'ae_epoch{}.pth'.format(epoch))
