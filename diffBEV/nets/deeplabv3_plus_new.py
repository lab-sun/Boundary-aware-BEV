...
from diffBEV.nets.utils_for_deeplab import get_visual_img, compute_losses, compute_results
...
class DeepLab(pl.LightningModule):
    def __init__(self, 
        ...
        #-----------------------------------------#
        # TODO ablation study
        self.aspp = ASPP(dim_in=in_channels, dim_out=256, rate=32//downsample_factor) # rate=16//downsample_factor
        ...
        self.encoder_out = nn.Conv2d(32, 4, 1, stride=1)
        self.decoder_in = nn.Conv2d(4, 32, 1, stride=1)
    ...
    def encoder(self, x): #将forward拆成encoder和decoder
        ...
        # 方案一：输出不同层级的特征  方案二：low_level_features先降维成x的大小，再送入decoder
        # 方案二：
    ...
    def forward(self, x):
        ...
        x = self.encoder(x) # torch.Size([16, 4, 32, 32])
        x = self.decoder(x) # torch.Size([16, 7, 150, 150])
        return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        FV_image = images.squeeze(dim=1).float()
        ...
        # 计算loss
        # import pdb; pdb.set_trace()
        # losses = self.seg_loss_fn(self.opt, seg_pred, BEV_labels, x, bev_images) # 有感知损失
        losses = self.seg_loss_fn(self.opt, seg_pred, BEV_labels) # 没有感知损失
        loss = losses['seg'] + losses['preceptual']
        self.log('Step_loss/train_seg', losses['seg'], on_step=True, prog_bar=True, logger=True)
        self.log('Step_loss/train_preceptual', losses['preceptual'], on_step=True, prog_bar=True, logger=True)
        self.log('Step_loss/train_loss', loss, on_step=True, prog_bar=True, logger=True)
        # 计算混淆矩阵
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 计算loss
        #losses = self.seg_loss_fn(self.opt, seg_pred, BEV_labels, x, bev_images)
        losses = self.seg_loss_fn(self.opt, seg_pred, BEV_labels)
        loss = losses['seg'] + losses['preceptual']
        self.log('Step_loss/train_seg', losses['seg'], on_step=True, prog_bar=True, logger=True)
        self.log('Step_loss/train_preceptual', losses['preceptual'], on_step=True, prog_bar=True, logger=True)
        self.log('Step_loss/train_loss', loss, on_step=True, prog_bar=True, logger=True)
    ...
    def training_epoch_end(self, outputs):
        ...
        self.showPred(images.squeeze(dim=1), out, bev_images, mode='Train')
    ...
    def validation_epoch_end(self, outputs):
        ...
        with torch.no_grad():
            out = self(images.squeeze(dim=1).to(self.device))
        self.showPred(images.squeeze(dim=1), out, bev_images, mode='Val')
