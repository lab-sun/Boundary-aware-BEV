...
class AutoEncoder(nn.Module):
    def __init__(self,
                ...
                # TODO ablation study
                pretrain_weigth = "./pretrain_weights/ae_ablation/2023-12-25-14-49(16*16*4-18)/ae_epoch18.pth",
                ):
        """
        ...
        # TODO ablation study
        cond_pretrain_weight = './pretrain_weights/deeplab_ablation/2023-12-22-08-39(16*16*4)/best-deeplab-epoch26-val_iou0.35.ckpt'  # lss model
        self.model_cond = DeepLab.load_from_checkpoint(cond_pretrain_weight, opt=opt, train_dataset=train_ds, downsample_factor=32).to('cuda')  # TODO ablation study  downsample_factor=16
...
class LatentDiffusion(pl.LightningModule):
    def __init__(self,
        ...
        self.model, self.diffusion = create_model_and_diffusion(
            **args_to_dict(opt.defaults, model_and_diffusion_defaults().keys())
        )
        self.model.to(dist_util.dev())
        self.schedule_sampler = create_named_schedule_sampler(opt.defaults['schedule_sampler'], self.diffusion,  maxt=1000)
...
class LatentDiffusionConditional(LatentDiffusion):
    ...
    def configure_optimizers(self):        
        ...
        param_group = [
            {'params': list(self.model.parameters()), 'weight_decay': self.opt.defaults['weight_decay'], 'lr': self.opt.defaults['lr']}, # 1e-4
